...
class ProjectMetaAction(AbstractAction):
    ...
    def logic(self, message, intent, entities):
        ...
        if intent=='deny':
            self.context.add_bot_msgs([Utils.chat_message(messages.keep_meta)])
            return KeepAction(self.context), False
        else:
...
class ProjectRegionAction(AbstractAction):
    ...
    def logic(self, message, intent, entities):
        ...
        if intent == 'deny':
            self.context.add_bot_msgs([Utils.chat_message(messages.keep_region)])
            return KeepAction(self.context), False
        else:
