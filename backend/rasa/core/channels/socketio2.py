...
class SocketIOInput(InputChannel):
    ...
    def blueprint(
        ...
        async def connect(sid: Text, data: Optional[Dict]) -> None:
            logger.debug(f"User {sid} connected to socketIO endpoint.")
            logger.debug(f"User {data} connected to socketIO endpoint.")
            global old

            if(data["HTTP_COOKIE"].split("=")[1] == old):
                old=sid
                logger.debug("sessione persistente")

            else:
                old=sid
                logger.debug("sessione nuova")
        ...
        async def session_request(sid: Text, data: Optional[Dict]):
         ...
         #           data["session_id"] = x.split("=")[1]
            ...
            logger.debug(f" Session request User {sid} connected to socketIO endpoint.")
