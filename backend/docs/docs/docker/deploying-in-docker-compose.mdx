...
The `expose: 5990` is what allows the `rasa` service to reach the `app` service on that port.
...
You should then be able to interact with your bot via requests to port 5990, on the webhook endpoint that
corresponds to a [configured channel](#configuring-channels):

```bash
curl -XPOST http://localhost:5990/webhooks/rest/webhook \
