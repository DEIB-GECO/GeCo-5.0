...
from data_structure.frame import Frame
...
class Payload:
    ...
    def update_status(self, dict):
        #old = self.status.copy()
        for k, v in dict.items():
            if k in self.status:
                old = self.status[k].copy()
                if isinstance(self.status[k], list):
                    self.status[k].append(v)
                elif isinstance(self.status[k], dict):
                    self.status[k].update(v)
            else:
                old = None
                if isinstance(v, list):
                    ...
                    self.status[k] = [v]
            self.context.top_delta().update_value(k, old, self.status[k])
            #if k in self.context.frame.keys():
            #   self.context.frame[k]['user'] = v
            #   print('set', k, dict[k])
...
class Context:
    ...
    def modify_status(self, dict):
        self.payload.update_status(dict)
